// Copyright © 2024-2025 Apple Inc. — Ported to C++

#include <mlx-lm/embedders/qwen3_embed.h>
#include <mlx-lm/common/activations.h>
#include <mlx-lm/common/attention_utils.h>
#include <cmath>

namespace mx = mlx::core;

namespace mlx_lm {

void from_json(const nlohmann::json& j, Qwen3EmbedConfiguration& c) {
    c.hidden_size = j.at("hidden_size").get<int>();
    c.num_hidden_layers = j.at("num_hidden_layers").get<int>();
    c.intermediate_size = j.at("intermediate_size").get<int>();
    c.num_attention_heads = j.at("num_attention_heads").get<int>();
    c.rms_norm_eps = j.at("rms_norm_eps").get<float>();
    c.vocab_size = j.at("vocab_size").get<int>();
    c.num_key_value_heads = j.at("num_key_value_heads").get<int>();
    c.rope_theta = j.value("rope_theta", 1000000.0f);
    c.head_dim = j.at("head_dim").get<int>();
    c.tie_word_embeddings = j.value("tie_word_embeddings", false);

    if (j.contains("rope_scaling") && !j["rope_scaling"].is_null()) {
        std::unordered_map<std::string, StringOrNumber> scaling;
        for (auto& [key, val] : j["rope_scaling"].items()) {
            StringOrNumber sn;
            from_json(val, sn);
            scaling[key] = sn;
        }
        c.rope_scaling = scaling;
    }
}

static mx::array linear_fwd(const mx::array& x, const mx::array& w) {
    return mx::matmul(x, mx::transpose(w));
}

// --- Qwen3EmbedAttention ---

Qwen3EmbedAttention::Qwen3EmbedAttention(const Qwen3EmbedConfiguration& config)
    : num_heads_(config.num_attention_heads),
      num_kv_heads_(config.num_key_value_heads),
      head_dim_(config.head_dim),
      scale_(std::pow(static_cast<float>(config.head_dim), -0.5f)),
      wq_weight_(mx::zeros({config.num_attention_heads * config.head_dim, config.hidden_size})),
      wk_weight_(mx::zeros({config.num_key_value_heads * config.head_dim, config.hidden_size})),
      wv_weight_(mx::zeros({config.num_key_value_heads * config.head_dim, config.hidden_size})),
      wo_weight_(mx::zeros({config.hidden_size, config.num_attention_heads * config.head_dim})),
      q_norm_weight_(mx::ones({config.head_dim})),
      k_norm_weight_(mx::ones({config.head_dim})),
      rms_norm_eps_(config.rms_norm_eps),
      rope_traditional_(false),
      rope_theta_(config.rope_theta),
      rope_scale_(1.0f)
{
    if (config.rope_scaling.has_value()) {
        auto& scaling = config.rope_scaling.value();
        auto type_it = scaling.find("type");
        if (type_it != scaling.end() && type_it->second.is_string() && type_it->second.as_string() == "linear") {
            auto factor_it = scaling.find("factor");
            if (factor_it != scaling.end() && factor_it->second.is_float()) {
                rope_scale_ = 1.0f / factor_it->second.as_float();
            }
        }
    }
}

mx::array Qwen3EmbedAttention::operator()(const mx::array& x,
                                            const std::optional<mx::array>& mask) {
    int B = x.shape(0), L = x.shape(1);

    auto queries = linear_fwd(x, wq_weight_);
    auto keys = linear_fwd(x, wk_weight_);
    auto values = linear_fwd(x, wv_weight_);

    // Reshape and apply Q/K norms
    queries = mx::reshape(queries, {B, L, num_heads_, -1});
    queries = mx::fast::rms_norm(queries, q_norm_weight_, rms_norm_eps_);
    queries = mx::transpose(queries, {0, 2, 1, 3});

    keys = mx::reshape(keys, {B, L, num_kv_heads_, -1});
    keys = mx::fast::rms_norm(keys, k_norm_weight_, rms_norm_eps_);
    keys = mx::transpose(keys, {0, 2, 1, 3});

    values = mx::transpose(mx::reshape(values, {B, L, num_kv_heads_, -1}), {0, 2, 1, 3});

    // Apply RoPE (no cache for embeddings)
    queries = mx::fast::rope(queries, head_dim_, rope_traditional_, rope_theta_, rope_scale_, 0);
    keys = mx::fast::rope(keys, head_dim_, rope_traditional_, rope_theta_, rope_scale_, 0);

    auto output = mx::fast::scaled_dot_product_attention(queries, keys, values, scale_, "", mask);
    output = mx::reshape(mx::transpose(output, {0, 2, 1, 3}), {B, L, -1});
    return linear_fwd(output, wo_weight_);
}

std::unordered_map<std::string, mx::array*> Qwen3EmbedAttention::weight_map() {
    return {
        {"q_proj.weight", &wq_weight_},
        {"k_proj.weight", &wk_weight_},
        {"v_proj.weight", &wv_weight_},
        {"o_proj.weight", &wo_weight_},
        {"q_norm.weight", &q_norm_weight_},
        {"k_norm.weight", &k_norm_weight_},
    };
}

// --- Qwen3EmbedMLP ---

Qwen3EmbedMLP::Qwen3EmbedMLP(int dimensions, int hidden_dimensions)
    : gate_weight_(mx::zeros({hidden_dimensions, dimensions})),
      down_weight_(mx::zeros({dimensions, hidden_dimensions})),
      up_weight_(mx::zeros({hidden_dimensions, dimensions}))
{}

mx::array Qwen3EmbedMLP::operator()(const mx::array& x) {
    auto g = linear_fwd(x, gate_weight_);
    auto up = linear_fwd(x, up_weight_);
    return linear_fwd(swiglu(g, up), down_weight_);
}

std::unordered_map<std::string, mx::array*> Qwen3EmbedMLP::weight_map() {
    return {
        {"gate_proj.weight", &gate_weight_},
        {"down_proj.weight", &down_weight_},
        {"up_proj.weight", &up_weight_},
    };
}

// --- Qwen3EmbedTransformerBlock ---

Qwen3EmbedTransformerBlock::Qwen3EmbedTransformerBlock(const Qwen3EmbedConfiguration& config)
    : attention_(config),
      mlp_(config.hidden_size, config.intermediate_size),
      input_layernorm_weight_(mx::ones({config.hidden_size})),
      post_attention_layernorm_weight_(mx::ones({config.hidden_size})),
      rms_norm_eps_(config.rms_norm_eps)
{}

mx::array Qwen3EmbedTransformerBlock::operator()(const mx::array& x,
                                                   const std::optional<mx::array>& mask) {
    auto r = attention_(mx::fast::rms_norm(x, input_layernorm_weight_, rms_norm_eps_), mask);
    auto h = mx::add(x, r);
    r = mlp_(mx::fast::rms_norm(h, post_attention_layernorm_weight_, rms_norm_eps_));
    return mx::add(h, r);
}

std::unordered_map<std::string, mx::array*> Qwen3EmbedTransformerBlock::weight_map() {
    std::unordered_map<std::string, mx::array*> map;
    for (auto& [k, v] : attention_.weight_map()) map["self_attn." + k] = v;
    for (auto& [k, v] : mlp_.weight_map()) map["mlp." + k] = v;
    map["input_layernorm.weight"] = &input_layernorm_weight_;
    map["post_attention_layernorm.weight"] = &post_attention_layernorm_weight_;
    return map;
}

// --- Qwen3EmbedModelInner ---

Qwen3EmbedModelInner::Qwen3EmbedModelInner(const Qwen3EmbedConfiguration& config)
    : embed_tokens_weight_(mx::zeros({config.vocab_size, config.hidden_size})),
      norm_weight_(mx::ones({config.hidden_size})),
      rms_norm_eps_(config.rms_norm_eps)
{
    layers_.reserve(config.num_hidden_layers);
    for (int i = 0; i < config.num_hidden_layers; ++i)
        layers_.emplace_back(config);
}

mx::array Qwen3EmbedModelInner::operator()(const mx::array& inputs) {
    auto h = mx::take(embed_tokens_weight_, inputs, 0);

    // Create attention mask for embeddings (bidirectional, no causal)
    // No mask needed for embedding models — they use full attention
    std::optional<mx::array> mask = std::nullopt;

    for (size_t i = 0; i < layers_.size(); ++i) {
        h = layers_[i](h, mask);
    }
    return mx::fast::rms_norm(h, norm_weight_, rms_norm_eps_);
}

std::unordered_map<std::string, mx::array*> Qwen3EmbedModelInner::weight_map() {
    std::unordered_map<std::string, mx::array*> map;
    map["embed_tokens.weight"] = &embed_tokens_weight_;
    map["norm.weight"] = &norm_weight_;
    for (size_t i = 0; i < layers_.size(); ++i) {
        auto prefix = "layers." + std::to_string(i) + ".";
        for (auto& [k, v] : layers_[i].weight_map()) map[prefix + k] = v;
    }
    return map;
}

// --- Qwen3EmbedModel ---

Qwen3EmbedModel::Qwen3EmbedModel(const Qwen3EmbedConfiguration& config)
    : config_(config), model_(config), vocabulary_size_(config.vocab_size)
{
    kv_heads_.resize(config.num_hidden_layers, config.num_key_value_heads);
}

EmbeddingModelOutput Qwen3EmbedModel::call_impl(
    const mx::array& inputs,
    const std::optional<mx::array>& /*position_ids*/,
    const std::optional<mx::array>& /*token_type_ids*/,
    const std::optional<mx::array>& /*attention_mask*/)
{
    auto out = model_(inputs);
    return {out, std::nullopt};
}

std::unordered_map<std::string, mx::array>
Qwen3EmbedModel::sanitize_impl(std::unordered_map<std::string, mx::array> weights) {
    std::unordered_map<std::string, mx::array> result;
    for (auto& [key, val] : weights) {
        if (key.find("self_attn.rotary_emb.inv_freq") != std::string::npos) continue;
        if (key.find("lm_head") != std::string::npos) continue;

        std::string k = key;
        if (k.substr(0, 6) != "model.") {
            k = "model." + k;
        }
        result.insert_or_assign(k, val);
    }
    return result;
}

void Qwen3EmbedModel::load_weights(const std::unordered_map<std::string, mx::array>& weights) {
    auto wmap = weight_map();
    for (auto& [name, target] : wmap) {
        auto it = weights.find(name);
        if (it != weights.end()) *target = it->second;
    }
}

std::unordered_map<std::string, mx::array*> Qwen3EmbedModel::weight_map() {
    std::unordered_map<std::string, mx::array*> map;
    for (auto& [k, v] : model_.weight_map()) map["model." + k] = v;
    return map;
}

} // namespace mlx_lm
