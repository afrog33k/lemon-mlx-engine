...
DECODE_TOKEN_BUDGETS="${DECODE_TOKEN_BUDGETS:-$MAX_TOKENS}"
JOBS="${JOBS:-8}"

LEMON_4BIT_MODEL="${LEMON_4BIT_MODEL:-mlx-community/Qwen3-0.6B-4bit}"
HIPFIRE_MODEL="${HIPFIRE_MODEL:-qwen3:0.6b}"
...
OUT_DIR="$OUT_BASE/qwen3_0p6b_vs_hipfire_$stamp"
mkdir -p "$OUT_DIR"
...
run_lemon_native() {
    ...
    set +e
    timeout "$TIMEOUT_SECONDS" env -u LEMON_MLX_DEQUANTIZE_WEIGHTS "$BENCH_BIN" "$LEMON_4BIT_MODEL" \
        --prompt "$prompt" \
        --raw \
...
awk -F '\t' \
    -v title="Qwen3 0.6B 4-bit vs hipfire" \
