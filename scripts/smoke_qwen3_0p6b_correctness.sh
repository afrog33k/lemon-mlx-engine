...
JOBS="${JOBS:-8}"
RUN_BF16="${RUN_BF16:-1}"
RUN_DEQUANTIZED="${RUN_DEQUANTIZED:-1}"
REQUIRE_NATIVE_DEQUANT_HASH_MATCH="${REQUIRE_NATIVE_DEQUANT_HASH_MATCH:-1}"

QWEN3_BF16_MODEL="${QWEN3_BF16_MODEL:-Qwen/Qwen3-0.6B}"
QWEN3_4BIT_MODEL="${QWEN3_4BIT_MODEL:-mlx-community/Qwen3-0.6B-4bit}"
...
OUT_DIR="$OUT_BASE/qwen3_0p6b_correctness_$stamp"
mkdir -p "$OUT_DIR"
...
stop_after_substring=$STOP_AFTER_SUBSTRING
qwen3_bf16_model=$QWEN3_BF16_MODEL
qwen3_4bit_model=$QWEN3_4BIT_MODEL
run_bf16=$RUN_BF16
run_dequantized=$RUN_DEQUANTIZED
require_native_dequant_hash_match=$REQUIRE_NATIVE_DEQUANT_HASH_MATCH
bf16_dequantize_weights=unset
native_dequantize_weights=unset
dequantized_dequantize_weights=1
...
run_case() {
    ...
    for ((run = 1; run <= RUNS; run += 1)); do
        ...
        case "$quant_mode" in
            bf16)
                env_args=(env -u LEMON_MLX_DEQUANTIZE_WEIGHTS)
                ;;
            native_quantized)
                env_args=(env -u LEMON_MLX_DEQUANTIZE_WEIGHTS)
                ;;
            dequantized)
                env_args=(env -u LEMON_MLX_DEQUANTIZE_WEIGHTS LEMON_MLX_DEQUANTIZE_WEIGHTS=1)
                ;;
            *)
        ...
        set +e
        timeout "$TIMEOUT_SECONDS" "${env_args[@]}" "$BENCH_BIN" "$QWEN3_4BIT_MODEL" \
        ...
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
            "$name" "$QWEN3_4BIT_MODEL" "$quant_mode" "$prompt_md5" "$bench_md5" "$run" "$rc" "$parsed" "$log" >> "$summary"
...
any_failed=0
if [[ "$RUN_BF16" != "0" ]]; then
    run_case "qwen3_0p6b_bf16" "$QWEN3_BF16_MODEL" "bf16" || any_failed=1
fi
run_case "qwen3_0p6b_4bit_native" "native_quantized" || any_failed=1
if [[ "$RUN_DEQUANTIZED" != "0" ]]; then
    run_case "qwen3_0p6b_4bit_dequantized" "dequantized" || any_failed=1
fi
...
if [[ "$RUN_DEQUANTIZED" != "0" && "$REQUIRE_NATIVE_DEQUANT_HASH_MATCH" != "0" ]]; then
    if ! awk -F '\t' '
        NR == 1 { next }
        $1 == "qwen3_0p6b_4bit_dequantized" && $7 == 0 && $8 == "ok" {
            dequant[$6] = $11
            dequant_tokens[$6] = $12
        }
        $1 == "qwen3_0p6b_4bit_native" && $7 == 0 && $8 == "ok" {
            native[$6] = $11
            native_tokens[$6] = $12
        }
        END {
